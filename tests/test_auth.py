...
class TestRouter(unittest.TestCase):
    ...
    def test_register_user(self):
        response = self.client.post("http://localhost:5000/user/auth/register",
                                    json={"username": self.fake.sentence(), "password": "wrong"})
        self.assertEqual(response.status_code, 200)
    ...
    def test_login_with_invalid_credentials(self):
        response = self.client.post("http://localhost:5000/user/auth/login",
